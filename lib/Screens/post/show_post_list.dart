...
class ShowPostList extends StatefulWidget {
  ...
  ShowPostList({required this.status});
  @override
  State<ShowPostList> createState() => _ShowPostListState();
}
...
class _ShowPostListState extends State<ShowPostList> {
  ...
  Future<List<UserPost>> getUserMatchPost(String userId) async {
    ...
    if (widget.status == 1) {
      response = await Apis().getAllPost(userId);
    } else if (widget.status == 2) {
      response = await Apis().getMatchingPost(userId);
    } else if (widget.status == 3) {
      response = await Apis().getAppliedPostList(userId);
    } else {
      response = await Apis().getFavouriteJob(userId);
    }
  ...
  Future addToFavorurite(String? id) async {
    ...
    if (statusCode == 200) {
      ...
      if (re == "success") {
        MyToast(message: msg).toast;
        if (msg == "Job Wislist Remove Successfully") {
